...
/* ESERCIZIO 6
 ...
 while(z<10)
 {
  massaTot+=Number(starWarsCharacters[z].mass);
  z++;
 }
 console.log("la somma delle masse è: "+massaTot);
...
for(let personaggio of starWarsCharacters)
{
  ...
  {
    if(personaggio.name==female.name)
    
    starWarsCharacters.splice(starWarsCharacters.indexOf());
  }
}
...
*/
{
  var rand = Math.floor(Math.random()*starWarsCharacters.length);
  console.log(starWarsCharacters[rand]);
}